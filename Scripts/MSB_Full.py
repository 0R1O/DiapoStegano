...
for i in range(h):
    for j in range(w):
        c = pxs[i*w+j] # Current
        r = c[0]%128 + 128*int(lorem[x%l]) # On force MSB à 0 puis on ajoute
        g = c[1]%128 + 128*int(lorem[(x+1)%l]) # le bon MSB en fonction de
        b = c[2]%128 + 128*int(lorem[(x+2)%l]) # string (lettre courrante)
...
for i in range(h):
    for j in range(w):
        c = pxs[i*w+j] # Current
        r = c[0]%128 + 128*int(binary[x]) # On force MSB à 0 puis on ajoute
        g = c[1]%128 + 128*int(binary[x+1]) # le bon MSB en fonction de
        b = c[2]%128 + 128*int(binary[x+2]) # binary (lettre courrante)
        newdata.append((r,g,b))
        x += 3
...
new.show()
new.save("MSB_BigText_RedOnly.png")
...
new.show()
new.save("MSB_BigText_GreenOnly.png")
...
new.show()
new.save("MSB_BigText_BlueOnly.png")
